...
def resolve_dataset_path():
    ...
    direct = os.path.join(os.getcwd(), "output_dataset.json")
    if os.path.exists(direct):
        return direct

    for root, _, files in os.walk(os.getcwd()):
        if "output_dataset.json" in files:
            return os.path.join(root, "output_dataset.json")

    raise FileNotFoundError("output_dataset.json not found")


def resolve_output_path():
    direct = os.path.join(os.getcwd(), "uctp_output.json")
    if os.path.exists(direct):
        return direct

    for root, _, files in os.walk(os.getcwd()):
        if "uctp_output.json" in files:
            return os.path.join(root, "uctp_output.json")
...
def evaluate(candidate_path: str):
    ...
    truth_path = resolve_dataset_path()
    ...
    try:
        ...
        SAVE_DIR = os.path.join(os.getcwd(), "saved_candidates")
    ...
    old_output = os.path.join(os.getcwd(), "uctp_output.json")
    ...
    pred_path = resolve_output_path()
    print("Resolved output path:", pred_path)
...
def evaluate_tracking(pred, ref, truth):
    ...
    #grab num_tracks from the count
    num_tracks = len(pred)

    #penalizing behaviour when it groups everything to one track 
    if num_tracks <= 1:
        combined_score *= 0.1

    # penalizing the false positives 
    fp_rate = FP / (TP + FP + 1e-8)
    combined_score -= 0.3 * fp_rate

    # clamping to avoid exponential growth 
