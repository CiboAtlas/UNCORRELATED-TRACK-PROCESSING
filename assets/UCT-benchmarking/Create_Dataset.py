...
if __name__ == "__main__":
    ...
    for bin in bins:
        ...
        #observations.to_csv("./data/observations_10_9.csv", index=False)
        #referenceSVs.to_csv("./data/referenceSVs_10_9.csv", index=False)
        #referenceTLEs.to_csv("./data/referenceTLEs_10_9.csv", index=False)

        # For each 'bin' (datasetCode), save a dataset to database
        # Save Created Dataset to a JSON file and save in database
        output_json = apiI.saveDataset(observations, TLEdataset, referenceSVs , referenceTLEs, './data/output_dataset_1021.json')
